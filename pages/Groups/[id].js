...
export async function getServerSideProps({ req, res }) {
  ...
  if (
    ...
    !result.get_success ||
    result.members[email] == undefined
  ) {
    return {
      props: {},
...
function splitEven() {
  if (typeof window !== "undefined") {
    ...
    if (
      input_total.value.search(/^[0-9]*[.][0-9]{2}$/g) == -1 &&
      input_total.value.search(/^[0-9]*$/g) == -1
    ) {
...
function selectPerson(event) {
  ...
  if (!elm.firstChild.classList.contains(`${styles.active}`)) {
    elm.lastChild.value = "";
    elm.parentNode.setAttribute(
      "count",
      parseInt(elm.parentNode.getAttribute("count")) - 1
    );
  } else {
    elm.parentNode.setAttribute(
      "count",
      ...
      <div className={styles.group_heading}>
        <div className={styles.group_info}>
          ...
          <div className={styles.group_amount}>
            <p style={{ color: hex }}>
              ${(amount < 0 ? amount * -1 : amount * 1).toFixed(2)}
            </p>
          </div>
        </div>
        <div className={styles.group_info}>
          <div className={styles.names_members}>
            <p>Join Link</p>
            <p className={styles.word_break_all}>
              localhost:8000/JoinGroup/{groupId}
            </p>
          </div>
        </div>
...
export default function Group({
    ...
    <>
      ...
      <main className={styles.main}>
        <div className={styles.transaction_history}>
          {pendingApproval.map((trans) => {
            ...
            if (trans.paid_to == userId) {
              return (
                <div
                  index={pending_num}
                  className={`${styles.transaction_container} ${styles.pending}`}
                  key={pending_num}
                  onClick={(e) => makePendingView(e)}
                >
          ...
          {expenseHistory.map((trans) => {
            ...
            let relative = isOwner
              ? sumMemberExpenses(trans.users)
              : (parseFloat(trans.users[userId]) || 0.0).toFixed(2);
            ...
            return (
              <div
                ...
                key={history_num}
                onClick={(e) => makeTransactionView(e)}
              >
          ...
          <button className={styles.delete_group_button} onClick={deleteGroup}>
            DELETE GROUP
          </button>
      ...
      <div className={styles.transaction_background} id="transaction_input">
        <div className={styles.transaction_large}>
          <div
            ...
            onClick={(e) =>
              hide(e.nativeEvent.target.parentNode.parentNode, true)
            }
          ></div>
          ...
          <div
            className={styles.transaction_people}
            count="0"
            id="transaction_people"
          >
              ...
              return (
                <div className={styles.person} key={id}>
                  ...
                  <input
                    ...
                    email={id}
                    onChange={(e) => {
                      e.target.parentNode.parentNode.nextElementSibling.style =
                        "";
                    }}
                  ></input>
                </div>
          ...
          <span></span>
          <div
            className={styles.submit_expense_container}
      ...
      <div className={styles.transaction_background} id="transaction_view">
        <div className={styles.transaction_large}>
          <div
            className={styles.x_button}
            onClick={(e) => hide(e.nativeEvent.target.parentNode.parentNode)}
          ></div>
          ...
          <div>
            <p className={styles.debt_remaining_text}>Debts Remaining</p>
          </div>
          <div
            className={styles.transaction_people}
            id="view_transaction_people"
          ></div>
          <div
            className={styles.submit_expense_container}
            onClick={(e) => {
              hide(e.nativeEvent.target.parentNode.parentNode);
              showFulfillExpense(e);
            }}
          >
            <p>Bill Me</p>
          </div>
      ...
      <div className={styles.transaction_background} id="pending_view">
        <div className={styles.transaction_large}>
          ...
          <div>
            <p
              className={`${styles.debt_remaining_text} ${styles.pending_larger_p}`}
            >
              Amount Paying
            </p>
          </div>
          <div
            ...
            onClick={(e) => {
              hide(e.nativeEvent.target.parentNode.parentNode);
              handlePendingPay(e, true);
            }}
          >
            <p>Accept</p>
          </div>
          <div
            className={`${styles.submit_expense_container} ${styles.negative}`}
            onClick={(e) => {
              hide(e.nativeEvent.target.parentNode.parentNode);
              handlePendingPay(e, false);
            }}
          >
            <p>Reject</p>
          </div>
      ...
      <div className={styles.transaction_background} id="submit_expense">
        <div className={styles.transaction_large}>
          ...
          <div className={styles.payment_method}>
            <button onClick={(e) => toggleBillVenmo(e, false)}>
              BillMates
            </button>
            <button onClick={(e) => toggleBillVenmo(e, true)}>Venmo</button>
          </div>
  ...
  function makeTransactionView(event) {
    ...
    for (let person in expense.users) {
      ...
      children_string += `<div class="${styles.person} ${
        styles.person_view
      }"><p>${members[person]}</p><p>$${amt_remaining.toFixed(2)}</p></div>`;
    }
    people_view.innerHTML = children_string;
    people_view.nextElementSibling.setAttribute(
      "index",
  ...
  function makePendingView(event) {
    ...
    heading.nextElementSibling.children[0].textContent =
      "Amount Paying: $" + expense.amount_paid.toFixed(2);
  ...
  async function handleExpenseSubmit() {
    ...
    let running_sum = 0;
    loooping: for (let i = 0; i < inputs.length; i++) {
      switch (i) {
        case 0:
        ...
        default:
          if (
            inputs[i].value.search(/^[0-9]*[.][0-9]{2}$/g) == -1 &&
            inputs[i].value.search(/^[0-9]*$/g) == -1
          ) {
    ...
    if (format.total == "" || parseFloat(format.total) <= 0) {
      ...
      format.total.search(/^[0-9]*[.][0-9]{2}$/g) == -1 &&
      format.total.search(/^[0-9]*$/g) == -1
    ) {
      ...
      elm.textContent =
        "Amount Difference: $" +
        Math.round((parseFloat(format.total) - running_sum) * 100) / 100;
      format.total = "";
    }
    ...
    if (format.title == "" || format.total == "" || format.expense == "") {
      ...
      console.log(format);
      format.total =
        Math.trunc(parseFloat(format.total) * 100) / 100 -
        Math.trunc((parseFloat(format.expense[userId]) || 0) * 100) / 100;
      ...
      if (result == LAMBDA_RESP.SUCCESS) {
        ...
        alert(
          "Check to make sure all inputs sum to the total, there are no '$', and that there is at least one person paying"
        );
        return;
      } else if (
        result == LAMBDA_RESP.ERROR ||
        result == LAMBDA_RESP.INVALID_TOKEN
      ) {
  ...
  function showFulfillExpense(event) {
    ...
    if (expenseHistory[index].owner == userId) {
      elm.children[0].children[2].children[0].textContent =
        "Would you like to clear this debt?";
      elm.children[0].children[2].children[1].value = sumMemberExpenses(
        expenseHistory[index].users,
        userId
      );
    } else {
      elm.children[0].children[1].children[
        JSON.parse(localStorage.getItem("payment_preference")) ==
        PAYMENT_PREFERENCE.VENMO
          ? 1
          : 0
      ].className = `${styles.selected_method}`;
      elm.children[0].children[1].children[
        JSON.parse(localStorage.getItem("payment_preference")) ==
        PAYMENT_PREFERENCE.VENMO
          ? 0
      ...
      elm.children[0].children[2].children[0].textContent =
        "Original Amount: " + expenseHistory[index].users[userId].toFixed(2);
      elm.children[0].children[2].children[1].value =
        expenseHistory[index].users[userId].toFixed(2);
    }
  ...
  async function handleExpensePay(event) {
    ...
    if (
      input.value == "" ||
      (input.value.search(/^[0-9]*[.][0-9]{2}$/g) == -1 &&
        input.value.search(/^[0-9]*$/g) == -1)
    ) {
      ...
      const result = await groupService.payDebt(
        userId,
        expenseHistory[index]._id,
        value
      );
      console.log(result);
      if (result == LAMBDA_RESP.SUCCESS) {
  ...
  async function handlePendingPay(event, isAccepted) {
    const expense =
      pendingApproval[event.target.parentNode.getAttribute("index")];
    let accepted = true;
    if (isAccepted) {
      console.log(expense);
    ...
    const result = await groupService.updatePendingState(
      isAccepted,
      expense.expense_id
    );
    if (result == LAMBDA_RESP.SUCCESS) {
  ...
  async function deleteGroup() {
    ...
    if (userId != manager) {
      alert("You are not the group manager");
      return;
    }
    if (name == null) {
