...
import {
  ...
  PayAllView,
  ReportView,
} from "./_components_/views.jsx";
import {
  ExpenseItem,
  PendingItem,
  ReportedItem,
} from "./_components_/items.jsx";
  ...
  const fetchData = async () => {
    ...
    if (response.errorType) {
      ...
      for (let i = 0; i < response.expenses.length; i++) {
        //have to push reported items to the front
      ...
      dispatch(
        groupDataAction.setGroupData({
          ...
          maxComment: response.maxComment,
      ...
      <>
        ...
        <main className={styles.main}>
                  ...
                  if (userId == item.paid_to) {
                    return (
                      <PendingItem
                        ...
                        owner={
                          response_data.members[item.paid_by] || item.paid_by
                        }
          ...
          {response_data.balance < 0 ? (
            <div
              className={styles.repay_all_container}
              onClick={() => {
                setPayAllVisible(true);
              }}
            >
              <p>Repay All</p>
            </div>
            ...
            <div
              className={styles.repay_all_container}
              onClick={() => {
                setWarningPopup(["You have no debts to pay", 1.5]);
              }}
            >
              <p>Repay All</p>
            </div>
        ...
        {currentPendingView != -1 ? (
          <PendingView
            members={response_data.members}
            expense={response_data.pending[currentPendingView]}
            hideParent={setCurrentPendingView}
          ></PendingView>
        ) : (
          <></>
        )}
        {currentTransactionView != -1 ? (
          <TransactionView
            userId={userId}
            members={response_data.members}
            expense={response_data.expenses[currentTransactionView]}
            hideParent={setCurrentTransactionView}
            showFulFill={setCurrentFulfillView}
          ></TransactionView>
        ) : (
          <></>
        )}
        {currentReportView != -1 ? (
          <ReportView
            userId={userId}
            members={response_data.members}
            expense={response_data.expenses[currentReportView]}
            hideParent={setCurrentReportView}
            showFulFill={setCurrentFulfillView}
          ></ReportView>
        ) : (
          <></>
        ...
        {transactionInputVisible ? (
          <TransactionInputView
            ...
            userId={userId}
            groupId={groupId}
            commentLength={response_data.maxComment}
            callback={setTransactionInputVisible}
            args={false}
          ></TransactionInputView>
        ) : (
          <></>
        ...
        {currentFulfillView != null ? (
          <FulFillView
            userId={userId}
            expense={currentFulfillView}
            owner={
              response_data.members[currentFulfillView.owner] ||
              currentFulfillView.owner
            }
            hideParent={setCurrentFulfillView}
            warningPopup={setWarningPopup}
            paymentAllowed={response_data.settings.fufillment}
          ></FulFillView>
        ) : (
          <></>
        )}
        {payAllVisible ? (
          <PayAllView
            balance={response_data.balance}
            userBalances={response_data.balances}
            members={response_data.members}
            userId={userId}
            groupId={groupId}
            commentLength={response_data.maxComment}
            callback={setPayAllVisible}
            args={false}
          ></PayAllView>
        ) : (
          <></>
        ...
        {warningPopup != null ? (
          <>
            <div
              className={styles.transaction_background}
              style={{ background: "transparent" }}
            >
              <div
                className={styles.warningPopup}
                style={{
                  animation: "popup " + warningPopup[1] + "s ease-in forwards",
                }}
              >
                <p>{warningPopup[0]}</p>
              </div>
