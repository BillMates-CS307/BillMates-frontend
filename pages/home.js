...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) =>
    async ({ req, res }) => {
      ...
      console.log(status);
      console.log(user);
      store.dispatch(userDataAction.setEmail({ email }));
      store.dispatch(userDataAction.setName({ name: user.name }));
      store.dispatch(userDataAction.setGroups({ groups: user.groups }));
      store.dispatch(
        userDataAction.setSettingsNotification({
          settings: { notification: user.settings.notification },
        })
      );
  ...
  function GroupTemplate({ groupName, debtOwed, groupId }) {
    return (
      ...
      >
        ...
        <p className={styles.debtInGroupP} style={{ background: hexBox }}>
          ${(debtOwed < 0 ? debtOwed * -1 : debtOwed * 1).toFixed(2)}
        </p>
      </div>
    );
  }
